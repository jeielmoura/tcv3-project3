...
def recursion (sess, i=0) :
	...
	if (i == NUM_NNS) and (np.sum(NNS_USED) > 0) :
		...
		better = better or (best_val_acc == val_acc and best_val_loss == val_loss and np.sum(NNS_USED) > best_nn_num)
	...
	for i in range (NUM_NNS) :
		if NNS_USED_BEST[i] :
			...
			for t in range( len(T) ) :
				ret = sess.run([out], feed_dict = { X: np.expand_dims(T[t], axis=0), is_training: False })
